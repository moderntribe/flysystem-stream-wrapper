...
class StreamWrapper {
	...
	/**
	 * @var \Tribe\Storage\Identity\Identifier
	 */
	...
	public function stream_open( ?string $path, ?string $mode, ?int $options, ?string &$opened_path ): bool {
		...
		if ( ! $this->stream->isSeekable() && ($options & STREAM_MUST_SEEK) ) {
			$this->stream = $this->cache_stream( $this->stream );
		}
	...
	public function stream_seek( int $offset, int $whence = SEEK_SET ): bool {
		if ( ! $this->stream->isSeekable() ) {
			// Convert to a stream that supports seeking
			$this->stream = $this->cache_stream( $this->stream );
		}
	 ...
	 * @param  \Tribe\Storage\Identity\Identifier  $identifier
	 ...
	 *
	 * @param  resource|string|int|float|bool|\Psr\Http\Message\StreamInterface|callable|\Iterator|null  $resource
	 * @param  array $options  {
	 *      Options
	 *
	 *      @type mixed[] $metadata custom metadata.
	 *      @type int     $size The size of the stream.
	 * }
	 *
	 * @return \GuzzleHttp\Psr7\CachingStream
	 */
	protected function cache_stream( $resource = '', array $options = [] ): CachingStream {
		return new CachingStream( Utils::streamFor( $resource, $options ) );
	}
	 ...
	 *
	 * @return \Tribe\Storage\Identity\Identifier
	 */
